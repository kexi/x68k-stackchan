...
出力バッファ:
    常に 768KB (`CGROM_SIZE` = $C0000) 固定長で、memmap.h の `kCgromSize`
    と同じ。先に全体を 0 で確保してから字形を書き込むので、埋めなかった
    領域は 0 のまま残り、出力サイズは入力の字数に依らない。

バイト順:
    **リトルエンディアンではない。** 1 行が 2 バイト以上の字形 (16x16 なら
    2 バイト) は、**左端のドットを含むバイトが先** (ビッグエンディアン相当)。
    68000 自体がビッグエンディアンで、IOCS は行の先頭バイトから順に
    読んで左から描く。ここを入れ替えると字の左右が 8 ドットずつ
    入れ替わった字形になる。BDF の BITMAP 行も同じ並びなので、
    16 進を上位バイトから切り出せばそのまま一致する。
...
def build_cgrom(bdf_dir: Path, output: Path) -> None:
    ...
    # 1 字も置けなかったら書き出さずに失敗させる。
    #
    # BDF は読めたのに字形が 1 つも配置できないのは、寸法や ENCODING が
    # 想定と違う (別サイズの BDF を渡した等) ということ。そのまま出すと
    # 中身が全部 0 の CGROM ができ、実機では「表示は出るが全部空白」に
    # なる。フォントが悪いのか描画が悪いのか切り分けられず、13 節の
    # 「画面が読めない」状態へ逆戻りする。
    #
    # Why not 警告だけ出して続けるか: 空の CGROM は使い道が無いうえ、
    # 出力ファイルがあると「生成できた」と見えてしまう。
    if kanji_written == 0 and ank_written == 0:
        raise ValueError(
            "字形を 1 つも配置できませんでした "
            f"({kanji_path.name} / {ank_path.name} の寸法と ENCODING を確認してください)"
        )
