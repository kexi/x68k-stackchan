...
// 1 ブロックの生成コードが取りうる最大バイト数。
//
// **runner の発行用の控え (staging_) の大きさはこれを使う。** 2 か所に
// 数字を置くと、片方だけ広げたときに「エミッタは吐けるが runner が
// 収まらないと言って捨てる」形ができ、被覆が黙って減る。
//
// 経緯: Tier A で 512 → Tier B (読みガード) で 1024 → Tier C (書きガード)
// で 1536。
//
// **Tier E (MOVEM) で 2560 へ広げた。** 書き形 MOVEM は 1 命令で最大 4 本を
// 転送し、転送ごとに「touch 2 組 + バイト 4 本の store」が乗る。実測の最悪
// (MOVEM.L <4 regs>,-(An) × 4) が 1985 バイトで、1536 では**収まらずに
// 黙って翻訳を諦めていた**。諦めた分は素通りするだけで正しさは損なわれない
// が、段 0-E がまさに狙っている形なので、そこを落とすと利得が消える。
// 余裕を 20% 以上取って 2560。**テストが最悪ケースを名指しで問う。**
inline constexpr size_t kMaxBlockBytes = 2560;
