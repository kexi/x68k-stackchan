...
public:
    ...
    // 6982 kHz。捨てる回数がそのまま速度に効く = **費用はホットセットを
    // 失うことであって、諦めている間の取りこぼしではない**。
    //
    // Why not もっと大きくしないか: 1000000 との差は 1.3% しかない一方、
    // 回復までの猶予は 10 倍違う。ほぼ全部の速度を取りながら生存性を
    // 残せる点を選ぶ。
    //
    // **public なのはテストが実値で上限を組み立てるため。** 定数を写すと
    // 値を変えたときにテストの前提だけ古くなる (実際に踏んだ)。
    static constexpr std::uint32_t kCapacityResetThreshold = 100000;
