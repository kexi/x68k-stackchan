...
//
// **上限は kMaxOps へ従属させる。** 動的分岐は末尾 1 つだけ (I6/G23) なので
// 最悪は「書き形が kMaxOps - 1 個 (3 本ずつ) + 末尾の JSR (4 本)」で、
// (kMaxOps - 1) * 3 + 4 本。
//
// **数値を書かない。** かつて kMaxOps = 4 に対して 16 と書いてあったが、
// kMaxOps を伸ばすとこの式が置き去りになり、溢れた形が**黙って諦められる**
// (素通りするだけなので正しさのテストでは永遠に落ちない)。段 0-E で
// staging が同じ形の罠を踏んでいる。kMaxOps から導けば置き去りにできない。
//
// 溢れたら failed で諦める。正しさは損なわれないが、諦めた分は
// 素通りするので気づきにくい。**テストが最悪ケースを名指しで問う。**
constexpr size_t kMaxPendingGuards = (kMaxOps - 1) * 3 + 4;
