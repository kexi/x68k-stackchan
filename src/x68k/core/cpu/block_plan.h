...
// ブロックへ入れる命令数の上限。
//
// **6 にしてある。実機で 4 / 5 / 6 / 8 / 12 を掃引して決めた。**
//
// | kMaxOps | 実効速度 | 命令/ブロック | 平均バイト | 常駐本数 |
// |---:|---:|---:|---:|---:|
// | 4 (旧) | 9073 kHz | 1.982 | 230 | 71 |
// | 5 | 9190 kHz | 2.106 | 236 | 69 |
// | **6** | **9310 kHz** | **2.164** | 244 | 67 |
// | 8 | 9220 kHz | 2.159 | 244 | 67 |
// | 12 | 9210 kHz | 2.164 | 245 | 66 |
//
// **命令/ブロックは 6 で飽和する。** 8 でも 12 でも 2.16 から動かない。
// 区間が切れる理由は分岐・呼出・復帰・未対応命令が主で、kCapacity は
// 主因ではないため、上限を上げても届かない。一方で常駐本数は単調に
// 減る (71 → 66) ので、6 より上は**得が飽和したまま損だけが伸びる**。
//
// Why not 旧コメントの「8 でも 16 でも実効的な差が出ない」を信じ続けないか:
// あれは段 0 の平均区間長 1.81 から引いた**机上の見積り**で、
// D = 218.3 サイクル (ブロック 1 本の往復の実費、ホストサイクルの 30.3%) を
// 実測する前のものだった。往復にそれだけ払っているなら、1.98 → 2.16 の
// 9% でも +2.6% になる。**「差が出ない」の根拠が実測で崩れた。**
//
// この上限は **割り込み受理の遅れの上限**でもある。ブロック実行中は
// デバイスに時間を渡さないので、新たに立った割り込みは最大でブロック
// 1 本ぶん遅れる。4 → 6 で上限が 1.5 倍になるが、受理が命令境界で
// 起きることは変わらない (test_block_plan.cpp の受理点のテストが問う)。
//
// sizeof(BlockPlan) は 152 バイト (PlannedOp 20 バイト × 6 + 鍵と出口 32)。
// 256 ブロックで 38,912 バイト、世代配列 4,096 バイトと合わせて 43,008 バイトで、
// 実測した内部 SRAM の空き 48,351 バイトに収まる (余裕 5,343 バイト)。
//
// **これ以上伸ばすときは 3 つを必ず確かめること:**
//   1. 受理点のずれを検査するテスト
//   2. sizeof(BlockPlan) × 256 + 4,096 が内部 SRAM の空きに収まるか
//   3. 最悪ケースが emit できるか。**Xtensa の条件分岐は ±2048 バイトしか
//      届かない** (canBranch12) ので、ガードから出口の島までがそれを超えると
//      requiredSize が 0 を返して**黙って翻訳を諦める** (素通りするだけなので
//      正しさのテストでは永遠に落ちない)。書き形 MOVEM ×5 が実際に届かない。
inline constexpr u32 kMaxOps = 6;
