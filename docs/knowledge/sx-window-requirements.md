...
**未解明**: ドライブ 0 が `$78` (Ready 込み) を返しているのに
`BTST #$1D` が成立しない理由。走査がドライブ 3→0 と降りる以上、
最後にドライブ 0 を見るはずで、そこでは Ready が立っている。
IOCS `$44` が ST3 をそのまま D0 に載せているか (`ROR.L #8` の前後で
何が入っているか) を、`$FF89F0` 以降で確かめるのが次の一歩。
