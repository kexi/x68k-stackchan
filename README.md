...
実効クロックは **7.6MHz**（実機 10MHz の 76%）。CoreS3 実機で Human68k を
動かした状態での実測値で、
...
速度改善です。現在 **実機比 76%**（7579 kHz）。目標の 50% は超えました
（[#4](https://github.com/kexi/x68k-stackchan/issues/4)）。

一番効いたのは **デバイスへ時間を渡す仕組みのイベント駆動化**で、単独で
**+51.9%**（4991 → 7579 kHz）。命令ごとに `tickDevices` を呼ぶのをやめ、
「次にデバイスの状態が変わる時点」まで飛ばします。実測では、律速は
68000 の命令実行（3%）ではなく `tickDevices`（59%）と割り込み判定（28%）
でした。**JIT は上限を実測して 1.03 倍にしかならないと分かったので却下
しています。**

quantum（N サイクルまとめて渡す）とは違います。次に状態が変わる時点へ
ちょうど着地し、レジスタを読む前に実体化・書く前に旧設定へ適用するので、
ゲストから見える値は毎命令 tick と完全に一致します。

「毎命令通る呼び出しを削る」型の最適化はほぼ枯れています。これ以上
インライン化を進めると呼び出し元のコードが膨らんで逆に遅くなります
（実測 -3.1%）。フラグ計算の遅延評価（上限 +0.3%）とデコード結果の
キャッシュ（同 +0.15%）も上限を測ってあり、残るのは動的再コンパイルだけ、
というところまで来ています。

ただし取りこぼしが 3 つありました。`Rtc::tick` / `Crtc::tick` /
`Mfp::tickTimerCounted` は毎命令通るのに別 TU の実呼び出しのままで、
実際に仕事をするのは 1 秒に 1 回・1 フレームに 2 回といった頻度でした。
速い側をヘッダへ出したところ、実機で **4828 → 5137 kHz（+6.40%）**。
quantum ではないので、渡すサイクル量も状態遷移も元のままです。

なお速度は**ゲストが何を実行しているかで変わります**。ディスク無しで
IPL-ROM のエラー画面を回しているときは同じ変更が +3.17% でした。
計測は必ず Human68k を起動した状態で行います。

あわせて、焼き直さずに実機で比較できるようにしました。シリアルの `'&'` で
最適化を ON/OFF できます。実効クロックは起動ごとに揺れるので、同じ起動の
中で切り替えないと揺れが効果と混ざるためです（音源の `'|'` と同じ形）。
