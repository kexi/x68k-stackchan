...
**M5Stack CoreS3 の実機で Human68k が起動し、`A>` プロンプトで `dir` が動きます。** ホスト（Mac）でも同じコードが動きます。
...
実効クロックは **10.1MHz**（実機 10MHz の 100.6%、実測 10063 kHz）。
CoreS3 実機で Human68k を動かした状態での実測値で、
メインメモリ 2MB・グラフィック画面の重ね合わせ・音源を全部載せた状態です。

一度 5.5MHz (55%) まで上げたとき、デバイスへ時間を渡す粒度を粗くした
...
返すので、LCD を見なくても画面の内容を確認できます。`|` で音源、`&` で
毎命令経路の最適化をそれぞれ ON/OFF でき、同じ起動の中で速度を比べられます。

### フォントについて

CGROM（漢字フォント）は無くても英数字は読めます。IPL-ROM 内蔵の 6×12 ANK フォントで代替する実装が入っているためです。

漢字を出したい場合は、東雲フォント（実質パブリックドメイン）から CGROM 相当の
イメージを生成できます。詳しくは [NOTICE.md](NOTICE.md) を参照してください。
...
**目標だった実機比 50%（[#4](https://github.com/kexi/x68k-stackchan/issues/4)）は
超え、実機 10MHz に到達しました。** 2 つの柱があります。

**1. デバイスへ時間を渡す仕組みのイベント駆動化** — 4842 → 8154 kHz（+68%）。
命令ごとに `tickDevices` を呼ぶのをやめ、「次にデバイスの状態が変わる時点」まで
飛ばします。実測では律速は 68000 の命令実行（3%）ではなく `tickDevices`（59%）と
割り込み判定（28%）でした。

quantum（N サイクルまとめて渡す）とは違います。次に状態が変わる時点へちょうど
着地し、レジスタを読む前に実体化・書く前に旧設定へ適用するので、ゲストから
見える値は毎命令 tick と完全に一致します。

**2. JIT（動的再コンパイル）** — 6970 → 10063 kHz。68000 の命令列を Xtensa の
ネイティブコードへ翻訳します。**かつて「上限を実測して 1.03 倍にしかならない」と
判断して却下していましたが、これは誤りでした。** 見積もりが 1 命令あたりの
固定費を過大に見ていて、実測の下限は 14.1 サイクルではなく 6.7 サイクルでした。

| 段 | 内容 | 速度 |
|---|---|---:|
| 開始 | | 6970 kHz |
| MOVEM / BSR / CMP・MOVEA・ADDA / 長さデコーダ | 翻訳できる命令を増やす | 9060 |
| kMaxOps 4 → 6 | 1 ブロックの命令数を増やす | 9358 |
| バイト照合 + スロット 2048 | 偽共有と衝突を同時に潰す | 9714 |
| 照合の控えをサイドテーブルへ | 長いブロックも照合できるように | **10063** |

被覆率（JIT が担う命令の割合）は 51.7% → 81.9%、1 ブロックあたり 1.50 → 2.16 命令。

**効かないと分かったことも記録しています。** admission ゲートと direct chaining は
実装して撤回、eviction は実装前に棄却しました。いずれも「正しい測定に基づいて
実装したのに、機構がその上限に届かなかった」形です。経緯は
[docs/knowledge/event-driven-implementation.md](docs/knowledge/event-driven-implementation.md)
にあります。

なお速度は**ゲストが何を実行しているかで変わります**。計測は必ず Human68k を
起動した状態で行います。焼き直さずに実機で比較できるよう、シリアルの `'&'` で
毎命令経路の最適化、`'J'` で JIT を ON/OFF できます。実効クロックは起動ごとに
揺れるので、同じ起動の中で切り替えないと揺れが効果と混ざるためです。
...
- ホストの単体テストは **774 test cases** が通る（`just test-host`）。
  ASan/UBSan 付きでも通る（`just test-san`）
- JIT は**変異テスト**で検証しています。各段で変異を注入し、テストが落ちることを
  確認しました。落ちない変異はテストの穴として塞いでいます
- 実機ファーム（ESP-IDF）のビルドが通る。バイナリは約 589KB
